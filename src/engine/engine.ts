...
export class DirectoryEngine {
  ...
  constructor(
    private structurePath: DirectoryStructureConfig['path'],
    private projectDestination: string
  ) {}
  ...
  private onSplitStreamData = async (input: string) => {
    ...
    this.currentPathSegments = this.currentPathSegments.slice(
      0,
      sanitizedInput.depth
    );
